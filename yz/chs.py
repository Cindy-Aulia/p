...
[[bold cyan]02[/]] check hasil crack cp    [[bold red]00[/]] kembali""",padding=(0,5),style="bold white", width=70))
        ...
        if xz in[""]:
            ...
            if len(self.hsl_ok)==0:
                ...
                input(f"   [ {O}KEMBALI{N} ] ");Xnxx("", "").kontol()
        elif xz in["2", "02"]:
            try:
...
[{biru_m}03{hapus}] kembali""", title=f"{merah}HAPUS HASIL CRACK{hapus}", padding=(0,5), style="bold white", width=70))
            ...
            if pil in ["1", "01"]:
                ...
                prints(Panel(f"[{hijau}✓{hapus}] berhasil menghapus semua hasil ok.", style="bold white", width=70));input(f"[ {O}TEKAN ENTER {N} ] ");Xnxx("", "").kontol()
            elif pil in ["2", "02"]:
                try:os.remove("results/CP")
                except:os.system("rm -rf results/CP")
                try:os.mkdir("results/CP")
                except:pass
                prints(Panel(f"[{hijau}✓{hapus}] berhasil menghapus semua hasil cp.", style="bold white", width=70));input(f"[ {O}TEKAN ENTER {N} ] ");Xnxx("", "").kontol()
            elif pil in ["3", "03"]:
                self.hasil()
            else:
